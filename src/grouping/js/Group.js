...
/**
 * Handles a Group
 * @class Y.WireIt.Group
 */    
Y.WireIt.Group = function(grouper, layer, serialisedGroup) {
...
Y.WireIt.Group.prototype = {
	...
	collapse: function(expanded)
	{
	    ...
	    var map = Y.WireIt.GroupUtils.getMap(this);
	    var collapsedConfig = Y.WireIt.GroupUtils.getCollapsedConfig(this, map);
	    var containers = [];
	    Y.WireIt.GroupUtils.addAllContainers(this, containers);
	    var sGroup = Y.WireIt.GroupUtils.serialiseGroup(this, containers);
	    
	    var modules = Y.WireIt.GroupUtils.getInternalModuleConfig(containers, collapsedConfig.center);
	    ...
	    var wires = Y.WireIt.GroupUtils.getWireConfig(this, getInternalContainerId, getExternalTerminalName);
	    ...
	    var gc = this.layer.addContainer(
				    {
						...
						groupConfig : {"group" : sGroup, "center": collapsedConfig.center, "modules" : modules, "wires" : wires.internal, "map" : Y.WireIt.GroupUtils.getExternalToInternalMap(map)},
		...
		Y.WireIt.GroupUtils.removeGroupFromLayer(this.groups[index].group, this.layer);
	...
	removeContainer: function(container, index)
	{
		...
		index = Y.WireIt.GroupUtils.firstTestSucess(this.containers, function (co) { return co.container == container; });
	...
	{
	    if (!Y.Lang.isValue(index))
		index = Y.WireIt.GroupUtils.firstTestSucess(this.groups, function (go) { return go.group == group });
	...
	generateUI: function(map, changedCallback)
	{
		...
		map = Y.WireIt.GroupUtils.getMap(this)
		...
		for (var cI in fieldsAndTerminals)
		{
			...
			if( fieldValue[fName] == undefined || ( fieldValue[fName] != undefined && fieldValue[fName] != "[wired]") ) { // Skip wired fields, we won't want them exposed.
			    fields[fName] = addField(fName, fMap, Y.WireIt.GroupUtils.valueOr(overrides[cI].overrides.fields[fName], {}), cI, fieldTerminals, showOnByIndex, showOffByIndex);
		    }
		    }
			...
			var tFragment = addTerminal(tName, tMap, Y.WireIt.GroupUtils.valueOr(overrides[cI].overrides.terminals[tName], {}), cI, fieldTerminals, showOnByIndex, showOffByIndex);
		...
		{ 
		    var containers = [];
		    Y.WireIt.GroupUtils.applyToContainers(self.groups[index].group, true, function(c) { containers.push(c) });
	...
	getAndCheckOverrides: function(containerUIMap, groupUIMap)
	{
	    ...
	    var overrides = Y.WireIt.GroupUtils.getOverridesFromUI(containerUIMap, groupUIMap);
	  ...
	  try {
			map = Y.WireIt.GroupUtils.getMap(tempGroup);
	  }
	  catch (err) {
