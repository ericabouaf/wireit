...
/**
 * @class Y.WireIt.Grouper
 */    
Y.WireIt.Grouper = function (layer, baseConfigFunction) {
	...
	this.rubberband = new Y.WireIt.RubberBand(this);
...
Y.WireIt.Grouper.prototype = {
   ...
   /**
    ...
    addContainer: function(container) {
		...
		if (Y.Lang.isValue(container.group)) {
	   	var group = Y.WireIt.GroupUtils.getOuterGroup(container.group);
	   	this.addGroup(group);
		}
    ...
    addGroup: function(group) {
		...
		if (this.groups.indexOf(group) == -1) {
	    	...
	    	var groupSelect = function(g) { Y.WireIt.GroupUtils.applyToContainers(g, true, function(c) { c.addedToGroup(); }); };
   ...
   removeGroup: function(group, index) {
		...
		if (index != -1) {
	    	...
	    	var containers = [];
	    	Y.WireIt.GroupUtils.addAllContainers(group, containers);
   ...
   makeGroup: function() {
		...
		if (this.containers.length > 0 || this.groups.length > 0) {
	    	var group = new Y.WireIt.Group(this, this.layer);
	    	var tempGroups = [];
	    	var tempContainers = [];
   ...
   showGroup: function(group, forceThisGroup) {
		...
		if (forceThisGroup)
	    	...
	    	g = Y.WireIt.GroupUtils.getOuterGroup(group);
   ...
   setupSelectedGroups: function(bottomGroup) {
		...
		Y.WireIt.GroupUtils.getOuterGroup(bottomGroup, function(current) {
   ...
   setSelectedGroup: function(group) {
		...
		Y.WireIt.GroupUtils.applyToContainers(this.selectedGroup, true, function(c) { c.highlight(); });
   },
   ...
   deselectGroup: function() {
		...
		if (Y.Lang.isValue(this.selectedGroup))
	    	Y.WireIt.GroupUtils.applyToContainers(this.selectedGroup, true, function(c) { c.dehighlight(); });
