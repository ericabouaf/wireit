(function() {
	...
	*/
	applyToContainers: function(group, deep, func, context)
	{
		...
		if (deep)
		{
			...
			Y.WireIt.GroupUtils.applyToContainers(group.groups[gI].group, deep, func, context);
		}
	    }
	},
	...
	*/
	removeGroupFromLayer : function(group, layer)
	{
		...
		for (var i in group.groups)
		    Y.WireIt.GroupUtils.removeGroupFromLayer(group.groups[i], layer);
	    }
	...
	*/
	addAllContainers: function(group, containers)
	{
		...
		for (var gI in group.groups)
		    Y.WireIt.GroupUtils.addAllContainers(group.groups[gI].group, containers)
	    }
	},
	...
	*/
	serialiseGroup: function(group, containers)
	{
		...
		for (var gI in group.groups)
		{
		    ...
		    sGroup.groups.push({"group" : Y.WireIt.GroupUtils.serialiseGroup(g.group, containers), "overrides" : g.overrides});
		}
	    }
	...
	* @param {object} map Optional The group's map (obtained by Y.WireIt.GroupUtils.getMap(group))
	* @return {object} The collapsed config
	*/
	getCollapsedConfig: function(group, map)
	{
	    if (!Y.Lang.isObject(map))
		map = Y.WireIt.GroupUtils.getMap(group);
	...
	*/
	workOutCenter: function(group)
	{
		...
		for (var gI in group.groups)
		{
		    ...
		    setBound(Y.WireIt.GroupUtils.workOutCenter(g));
		}
	    }
	    return [
	...
	*/
	...
	generateDefaultFieldNames : function(map, usedNames)
	{
		...
		for (var fName in c.fields)
		{
			...
			if (f.fieldConfig.wirable)
			{
			    ...
			    f.externalName = Y.WireIt.GroupUtils.generateFreshName(fName, mergedUsedNames);
			...
			{
			    f.externalName = Y.WireIt.GroupUtils.generateFreshName(fName, usedNames.fields);
	...
	generateTerminalMap: function(group, usedNames, containerMap, groupMap)
	{
	    ...
	    var allContainers = [];
	    Y.WireIt.GroupUtils.addAllContainers(group, allContainers);
	    var inGroup = function(container)
		{
		...
		var map = Y.WireIt.GroupUtils.getMap(g);
		var terminalConfigs = Y.WireIt.GroupUtils.getCollapsedConfig(g, map).terminals//TODO: inefficient 
		var forceVisible;
		...
		{
		    var externalToInternalMap = Y.WireIt.GroupUtils.getExternalToInternalMap(map);
	...
	generateFieldMap: function(group, usedNames, containerMap, groupMap)
	{
		...
		var map = Y.WireIt.GroupUtils.getMap(g);
		var fieldConfigs = Y.WireIt.GroupUtils.getCollapsedConfig(g, map).fields//TODO: inefficient since we throw away terminal results then get them again
		var forceVisible;
