 ...
 */
Y.WireIt.GroupFormContainer = function(options, layer) {
   /*var fieldConfigs = Y.WireIt.GroupUtils.fieldConfigsFromModules(options.groupConfig.modules, options.getBaseConfigFunction);
   
   var intermediate = Y.WireIt.GroupUtils.generateFields(fieldConfigs, {fields: {}, terminals: {}}, {fields: {"0email" : true}, terminals: {}}); //TODO: wrong arguments
   options.fields = intermediate.fields;
   var terminalConfigs = Y.WireIt.GroupUtils.terminalConfigsFromModules(options.groupConfig.modules, options.getBaseConfigFunction);
   
   options.terminals = Y.WireIt.GroupUtils.generateTerminals(terminalConfigs , {fields: {}, terminals: {}}, {fields: {}, terminals: {}}, intermediate.usedTerminalNames);
    */ 
   Y.WireIt.GroupFormContainer.superclass.constructor.call(this, options, layer);
   this.getBaseConfig = this.options.getBaseConfigFunction;
   
   this.positionTerminals();
};

Y.extend(Y.WireIt.GroupFormContainer, Y.WireIt.FormContainer, {
   
   /**
    * @method setOptions
    */
   setOptions: function(options) {
      Y.WireIt.GroupFormContainer.superclass.setOptions.call(this, options);
    ...
    */
   render: function() {
      Y.WireIt.GroupFormContainer.superclass.render.call(this);
      this.renderExpand();
   },
	...
	var horizontalMax = Math.max(Y.WireIt.GroupUtils.valueOr(terminals["top"], []).length, Y.WireIt.GroupUtils.valueOr(terminals["bottom"], []).length);
	var verticalMax = Math.max(Y.WireIt.GroupUtils.valueOr(terminals["left"], []).length, Y.WireIt.GroupUtils.valueOr(terminals["right"], []).length);
   ...
   renderExpand: function() {
	...
	//var expandedContainers = Y.WireIt.GroupUtils.expand(this.options.groupConfig, this, this.layer);
	...
	var deserialise = function(sGroup, groupToSet)
	{
		...
		group = new Y.WireIt.Group(this.group.grouper, this.layer)
...
/*
	...
	for (var fName in this.form.inputsNames)
	{
	    ...
	    var internal = Y.WireIt.GroupUtils.getInternalField(this.options.groupConfig, fName);
	...
	*/
	...
	for (var tI in this.terminals)
	{
	    ...
	    for (var wI in t.wires)
	    {
			...
			var internal = Y.WireIt.GroupUtils.getInternalTerminal(this.options.groupConfig, t.options.name);
	...
	*/
    ...
    getConfig : function()
    {
	var config = Y.WireIt.GroupFormContainer.superclass.getConfig.call(this);
