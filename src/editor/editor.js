...
Y.extend(Y.WireWiringEditor, Y.WireBaseEditor, {
	...
	renderModulesAccordion: function() {
		...
		if(!Y.one('modulesAccordionView')) {
			Y.one('left').appendChild( Y.Wirecn('ul', {id: 'modulesAccordionView'}) );
			var li = Y.Wirecn('li');
			li.appendChild(Y.Wirecn('h2',null,null,"Main"));
			var d = Y.Wirecn('div');
			d.appendChild( Y.Wirecn('div', {id: "module-category-main"}) );
			li.appendChild(d);
	...
	try {
		var div = Y.Wirecn('div', {className: "WiringEditor-module"});
      ...
      if(module.container.icon) {
         div.appendChild( Y.Wirecn('img',{src: module.container.icon}) );
      }
      div.appendChild( Y.Wirecn('span', null, null, module.name) );
