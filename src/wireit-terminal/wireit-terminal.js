YUI.add("wireit-terminal", function(Y){
 ...
 */
Y.WireIt.Terminal = function(parentEl, options, container) {
   ...
   if(this.editable) {
      ...
      //this.dd = new Y.WireIt.TerminalProxy(this, this.ddConfig);
      */
      
      this._makeEditable();
      
      this.scissors = new Y.WireIt.Scissors(this);
   }
};

Y.WireIt.Terminal.prototype = {
	...
	_makeEditable: function() {
		...
		drag.on('drag:start', function(ev) {
			...
			drag.wire = new Y.WireIt.BezierWire(
			   { 
...
Y.augment(Y.WireIt.Terminal, Y.EventTarget);
