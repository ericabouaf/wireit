YUI.add("wireit-terminal-proxy", function(Y){
 ...
 */
Y.WireIt.TerminalProxy = function(terminal, options) {
	...
	// Init the DDProxy
	/*Y.WireIt.TerminalProxy.superclass.constructor.call(this,this.terminal.el, undefined, {
...
Y.WireIt.TerminalProxy.prototype = {
	...
	startDrag: function() {
	   ...
	   if(this.terminal.container) {
		...
		var klass = Y.WireIt.wireClassFromXtype(this.terminal.editingWireConfig.xtype);
	   this.editingWire = new klass(this.terminal, this.fakeTerminal, parentEl, this.terminal.editingWireConfig);
	   
	   Y.one(this.editingWire.element).addClass(CSS_PREFIX+'Wire-editing');
	},
	...
	onDragDrop: function(e,ddTargets) {
	   ...
	   if(term2.alwaysSrc) {
		...
		var klass = Y.WireIt.wireClassFromXtype(term1.wireConfig.xtype);
